...
class Graph{
	public:
		...
		void printMatrix();
		std::vector<std::pair<int,int>> AStar(std::pair<int,int> start, std::pair<int, int> end, bool print);
		int heuristic(std::pair<int,int> one, std::pair<int,int> two);
		std::vector<std::pair<int,int>> dijkstra(std::pair<int,int> source, std::pair<int,int> target, bool print);
